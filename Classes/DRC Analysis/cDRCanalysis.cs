...
namespace HCSAnalyzer.Classes
{
    ...
    public class cDRC_Region
    {
        ...
        {
            List<int> ListRespondingDescriptors = new List<int>();

            for (int IdxDesc = 0; IdxDesc < CurrentScreen.ListDescriptors.Count; IdxDesc++)
            {
                cDescriptorType Desc = CurrentScreen.ListDescriptors[IdxDesc];
                if (!Desc.IsActive())
                {
                    ListRespondingDescriptors.Add(-1);
                    continue;
                }
            ...
            return ListRespondingDescriptors;
        }
        ...
        public cDRC GetDRC(cDescriptorType DescriptorType)
        {
    ...
    public class cDRC
    {
        public cDescriptorType AssociatedDescriptorType = null;
        ...
        public cDRC(cDRC_Region DRC_Region, cDescriptorType DescriptorType)
        {
        ...
        private void FitToSigmoid()
        {
            ...
            for (int IdxConc = 0; IdxConc < AssociatedDRCRegion.NumConcentrations; IdxConc++)
            {
                ...
                if (AssociatedDRCRegion.GetlistReplicate(0)[IdxConc].GetCurrentClassIdx() != -1)
                    ResultFit.ConcentrationValues.Add((double)AssociatedDRCRegion.GetlistReplicate(0)[IdxConc].ListProperties.FindValueByName("Concentration"));
            }
            ...
            for (int idxRep = 0; idxRep < AssociatedDRCRegion.NumReplicate; idxRep++)
            {
                ...
                for (int IdxConc = 0; IdxConc < AssociatedDRCRegion.NumConcentrations; IdxConc++)
                {
                    ...
                    if (AssociatedDRCRegion.GetlistReplicate(idxRep)[IdxConc].GetCurrentClassIdx() != -1)
                        ResultFit.Y_RawData[idxRep][IdxConc] = AssociatedDRCRegion.GetlistReplicate(idxRep)[IdxConc].ListSignatures[DescIdx].GetValue();
                    NumWellToProcess++;
                }
            }
