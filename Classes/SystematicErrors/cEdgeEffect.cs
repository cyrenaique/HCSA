...
namespace HCSAnalyzer.Classes
{
    public class cEdgeEffect
    {
        ...
        public cEdgeEffect(int MaxIteration)
        {
        ...
        {
            Mask = new double[cGlobalInfo.CurrentScreening.Columns + 2, cGlobalInfo.CurrentScreening.Rows + 2];

            for (int X = 0; X < cGlobalInfo.CurrentScreening.Columns + 2; X++)
            {
                Mask[X, 0] = 1;
                Mask[X, cGlobalInfo.CurrentScreening.Rows + 1] = 1;
            }
        ...
        public double[] FindBestShiftMultCoeff(double[,] inputPlate, int IdxDiff)
        {
            ...
            for (double DiffusionInitTemp = (double)cGlobalInfo.OptionsWindow.numericUpDownEdgeEffectMinMult.Value; DiffusionInitTemp < (double)cGlobalInfo.OptionsWindow.numericUpDownEdgeEffectMaxMult.Value; DiffusionInitTemp += (double)cGlobalInfo.OptionsWindow.numericUpDownEdgeEffectDeltaMult.Value)
            {
                for (double PlateInitTemp = (double)cGlobalInfo.OptionsWindow.numericUpDownEdgeEffectMinShift.Value; PlateInitTemp < (double)cGlobalInfo.OptionsWindow.numericUpDownEdgeEffectMaxShift.Value; PlateInitTemp += (double)cGlobalInfo.OptionsWindow.numericUpDownEdgeEffectDeltaShift.Value)
                {
