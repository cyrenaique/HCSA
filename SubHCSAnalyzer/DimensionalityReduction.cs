...
namespace HCSAnalyzer
{
    public partial class HCSAnalyzer
    {
        ...
        private void buttonReduceDim_Click(object sender, EventArgs e)
        {
            ...
            if (New_Dim >= cGlobalInfo.CurrentScreening.GetNumberOfActiveDescriptor())
            {
        ...
        {
            int NumClasses = cGlobalInfo.CurrentScreening.GetNumberOfClasses();
            if (NumClasses <= 1)
            {
            ...
            {
                cInfoClass InfoClass = cGlobalInfo.CurrentScreening.GetCurrentDisplayPlate().GetNumberOfClassesBut(NeutralClass);
                insts = cGlobalInfo.CurrentScreening.GetCurrentDisplayPlate().CreateInstancesWithClasses(InfoClass, NeutralClass);
            }
            else
            {
                cInfoClass InfoClass = cGlobalInfo.CurrentScreening.GetNumberOfClassesBut(NeutralClass);
                insts = cGlobalInfo.CurrentScreening.CreateInstancesWithClasses(InfoClass, NeutralClass);
            }

            cInfoDescriptors InfoDescriptors = cGlobalInfo.CurrentScreening.BuildInfoDesc();
            int RealIdx = 0;
