...
{
    public partial class IM3_Plugin3 : Plugin
    {
        public IM3_Plugin3()
            : base()
        {
        ...
        private void AnalyseSequence(Experiment CurrentExperiment, Sequence SeqToAnalyse)
        {
            ...
            {
                Current3DWorld = new c3DWorld(new cPoint3D(SeqToAnalyse[0].Width, SeqToAnalyse[0].Height, SeqToAnalyse[0].Depth), new cPoint3D((float)SeqToAnalyse.XResolution, (float)SeqToAnalyse.YResolution, (float)SeqToAnalyse.ZResolution));
            }
            else
            {
             //   Current3DWorld.ren1.
            ...
            {
                FormForControl Obj = ListFormForControl.ElementAt(ListFormForControl.Count - k-1);
                if (Obj.numericUpDownChannel.Value >= SeqToAnalyse.NumBands)
                {
           ...
           // Current3DWorld.ListObjControl = ListFormForControl;
            ...
            for (int k = 0; k < ListFormForControl.Count; k++)
            {
                ...
                if (Obj.radioButtonIsVolume.Checked)    // we have to detect a volume !
                {
                    ...
                    {
                        Obj.AssociatedBiologicalObjectList = Detection.IntensityThreshold((float)Obj.WindowForVolumeDetection.numericUpDownIntensityThreshold.Value, (float)Obj.WindowForVolumeDetection.numericUpDownVolumeMinVol.Value, (float)Obj.WindowForVolumeDetection.numericUpDownVolumeMaxVol.Value);
                    }
                    else // region growing
                    {
                        if (Obj.WindowForVolumeDetection.radioButtonRegionsBased.Checked) // start from a segmentation
                        {
                            if (Obj.WindowForVolumeDetection.radioButtonVolumeDetectionNew.Checked) // a new segmentation has to be performed
                            {
                                ...
                                Image3D CurrentBinary = new Image3D(SeqToAnalyse.Width, SeqToAnalyse.Height, SeqToAnalyse.Depth, 1);

                                for (int i = 0; i < CurrentBinary.ImageSize; i++)
                                {
                                    if (SeqToAnalyse[0].Data[(int)Obj.numericUpDownChannel.Value][i] >= (float)Obj.WindowForVolumeDetection.numericUpDownIntensityThreshold.Value)
                                        CurrentBinary.Data[0][i] = 1;
                                }

                                Connectivity CurrentConnectivity = Connectivity.TwoD_4;
                                if (CurrentBinary.Depth > 1)
                                    CurrentConnectivity = Connectivity.ThreeD_6;
                    ...
                    //if (Obj.WindowForVolumeDetection.radioButtonRegionsBased.Checked)
                    //{
                    //    if(Obj.WindowForVolumeDetection.checkBoxIsRegionGrowing.Checked)
                    //        Obj.AssociatedBiologicalObjectList = Detection.IntensityThreshold((float)Obj.WindowForVolumeDetection.numericUpDownIntensityThreshold.Value, (float)Obj.WindowForVolumeDetection.numericUpDownVolumeMinVol.Value, (float)Obj.WindowForVolumeDetection.numericUpDownVolumeMaxVol.Value);
                    //   // else
                    ...
                    if (Obj.AssociatedBiologicalObjectList != null)
                    {
                        ...
                        for (int i = 0; i < Obj.AssociatedBiologicalObjectList.Count; i++)
                        {
                            ...
                            if (Obj.checkIsBoxPointingArrow.Checked)
                                TmpVol.AddPointingArrow(Current3DWorld, (double)Obj.ForArrow.numericUpDownArrowScale.Value, Obj.ForArrow.buttonChangeColorPositive.BackColor);
                ...
                else if (Obj.RadioButtonIsSpot.Checked)
                {
                    ...
                    Obj.AssociatedBiologicalObjectList = Detection.HessianDetection((float)Obj.WindowForSpotDetection.numericUpDownSpotRadius.Value, (float)Obj.WindowForSpotDetection.numericUpDownIntensityThreshold.Value, (int)Obj.WindowForSpotDetection.numericUpDownSpotLocality.Value, (double)Obj.WindowForSpotDetection.numericUpDownSphereDisplayRadius.Value);
                    ...
                    for (int i = 0; i < Obj.AssociatedBiologicalObjectList.Count; i++)
                    {
                        ...
                        if (Obj.checkIsBoxPointingArrow.Checked)
                            TmpSpot.AddPointingArrow(Current3DWorld, (double)Obj.ForArrow.numericUpDownArrowScale.Value, Obj.ForArrow.buttonChangeColorPositive.BackColor);
            ...
            for (int k = 0; k < ListFormForControl.Count; k++)
            {
                ...
                foreach (cBiological3DObject CurrentSubObj in Obj.AssociatedBiologicalObjectList)
                {
                    cBiological3DObject ObjectToIdentify = null;
            ...
            for (int k = 0; k < ListFormForControl.Count; k++)
            {
              ...
              //  if (!Obj.IsMasterObject()) continue;
                ...
                {
                    Current3DWorld.AddGeometric3DObject(CurrentMeta.GenerateDelaunay(2.0f));
                }
            }
