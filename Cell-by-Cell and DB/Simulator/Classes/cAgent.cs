...
{
    [Serializable]
    public class cListAgents : List<cAgent>
    {
        public string Name;
        public string Information;


        public cListAgents(string Name)
        {
            this.Name = Name;
        }
        ...
        public cListExtendedTable GetAssociatedTables(cListCellType ListCellTypes)
        {
            cListExtendedTable ListTables = new cListExtendedTable();
            ...
            for (int i = 0; i < ListCellTypes.Count; i++)
            {
               ...
               // ListTables.Add(new cExtendedTable());
                ...
                cExtendedList ListX = new cExtendedList("X");
                ToReturn.Add(ListX);
                cExtendedList ListY = new cExtendedList("Y");
                ToReturn.Add(ListY);
                cExtendedList ListZ = new cExtendedList("Z");
                ToReturn.Add(ListZ);
                ...
                foreach (var item in this)
                {
                    if (item.Type == TmpCellType)
                    {
                        ...
                        ToReturn[0].Add(item.CentroidPosition.X);
                        ToReturn[1].Add(item.CentroidPosition.Y);
                        ToReturn[2].Add(item.CentroidPosition.Z);
                        ToReturn[3].Add(item.GetVolume());
                    }

                }


                ListTables.Add(ToReturn);
        ...
        public cExtendedTable GetAssociatedTable()
        {
            cExtendedTable ToReturn = new cExtendedTable();
            ToReturn.ListRowNames = new List<string>();
            ToReturn.ListTags = new List<object>();
    ...
    public class cInfoAgent
    {
        cAgent Cell;

        public cInfoAgent(cAgent Cell)
        {
            this.Cell = Cell;
        }
    ...
    public class cAgent
    {
        ...
        public static cWorld CurrentWorld;

        // public List<cAgent> PreviousStates = new List<cAgent>();

        cListAgents ListSubAgents = null;

        public bool MemoryOn = false;

        public cAgent(cCellType intialCellType, cPoint3D InitialPosition, double CurrentCyclePos)
        {
            BasicInit(intialCellType, InitialPosition);
            this.CurrentCyclePos = CurrentCyclePos;
        }

        public cAgent(cAgent CellToCopy)
        {
        ...
        public cAgent Divide()
        {
            cAgent NewAgent = new cAgent(this.Type,
                new cPoint3D(this.CentroidPosition.X + this.GetVolume() / 2,
                this.CentroidPosition.Y + this.GetVolume() / 2,
                this.CentroidPosition.Z + this.GetVolume() / 2), this.CurrentCyclePos);


            NewAgent.InitialVolume = this.InitialVolume;

            //NewAgent.CentroidPosition = new cPoint3D(this.CentroidPosition.X + this.GetVolume() / 2,
            //    this.CentroidPosition.Y + this.GetVolume() / 2,
            //    this.CentroidPosition.Z + this.GetVolume() / 2);
        ...
        public void RunSingleTick(Random RND, FormForSimuGenerator Parent)
        {
            if (MemoryOn)
            {
                // cAgent CopyCell = new cAgent(this);
            ...
            // Cell Division Test
            //   CurrentWorld.ListCells.Add(this.Divide());
